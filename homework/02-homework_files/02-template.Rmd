...
The code block below points to the packages you need to do the work discussed in the session. If you have been following the instructions from  the last session, you will have already installed the key packages -- `tidyverse` , `fst`, `lubridate`, `sf` -- and so simply need to make it available in session with `library(<package-name>)`. You will also need to **install** the [bikedata](https://github.com/ropensci/bikedata) package for accessing bikeshare data. We will install the development version with `install_github()` using the `devtools` package. We only use `devtools` in this case on setup and so access its functions explicitly with `::`, e.g.  `devtools::` rather than `library<package-name>`.
...
So run the code block below to download the extracted data to your project's data folder. Note that you should only run the import code below once. When returning to this work in a new R session, you need only then read in the `ny_trips.fst` and `ny_stations.csv` file that is held locally on your machines.
