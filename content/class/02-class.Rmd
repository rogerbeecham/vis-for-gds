...
# devtools::install_github("ropensci/bikedata") # API to TfL's trip data. Uncomment to run.
library(bikedata)
# install.packages(DBI)
# install.packages("SQLite")
```

```{r download-collected, echo=FALSE, eval=FALSE, message=FALSE}
tmp_file <- tempfile()
fst_url <- "https://www.roger-beecham.com/datasets/ny_stations.fst"
curl::curl_download(fst_url, tmp_file, mode="wb")
ny_stations <- fst::read_fst(tmp_file)
tmp_file <- tempfile()
fst_url <- "https://www.roger-beecham.com/datasets/ny_trips.fst"
curl::curl_download(fst_url, tmp_file, mode="wb")
...
{{% alert objective %}}
- [x] **Load** flat file datasets RStudio via querying an API.
...
In the module we will work with [data frames](http://adv-r.had.co.nz/Data-structures.html#data-frames) in R. These are spreadsheet like representations where rows are **observations** (case/record) and columns are **variables**. Each variable (column) in a data frame is a [vector](http://adv-r.had.co.nz/Data-structures.html#vectors) that must be of equal length. Where observations have missing values for certain variables, therefore where they may violate this equal-length requirement, the missing values must be substituted with something, usually with `NA` or similar. This constraint occasionally causes difficulties, for example when working with variables that contain values of different length for an observation. In these cases we create a special class of column, a [`list-column`](https://jennybc.github.io/purrr-tutorial/ls13_list-columns.html), something we'll return to later in the module.
...
kableExtra::kbl(variable_types,  caption = "A breakdown of @stevens_on_1946 variable types, operators and measures of central tendency that can be applied to each.", protect_latex = TRUE, escape=FALSE) %>%
 kableExtra::pack_rows("Categories", 1, 2, bold=FALSE, label_row_css = "border-bottom: 0px solid;") %>%
 kableExtra::pack_rows("Measures", 3, 4, bold=FALSE, label_row_css = "border-bottom: 0px solid;") %>%
...
kableExtra::kbl(drugs,  caption = "Table 1 of @wickham_tidy_2014.")
```
...
kableExtra::kbl(drugs,  caption = "Alternative organisation of Table 1 of @wickham_tidy_2014.")
```
...
kableExtra::kbl(gapminder_tidy,  caption = "Tidy excerpt of [`gapminder`](https://www.gapminder.org/data/) dataset.")
```
So the **variables**:
...
kableExtra::kbl(gapminder_untidy1,  caption = "Untidy excerpt of [`gapminder`](https://www.gapminder.org/data/) dataset: observations spread across rows")
```
...
kableExtra::kbl(gapminder_untidy2,  caption = "Untidy possible excerpt of [`gapminder`](https://www.gapminder.org/data/) dataset: observations spread across columns")
```
...
knitr::include_graphics("/class/02-class_files/view.png", error = FALSE)
```
...
kableExtra::kbl(data_types, caption = "A breakdown of data types in R.")
```
...
kableExtra::kbl(dplyr_verbs, caption = "dplyr funcitions (verbs) for manipulating data frames.")
```
...
kableExtra::kbl(aggregate_functions, caption = "A breakdown of aggregate functions commonly used with `summarise()`.")
```
...
knitr::include_graphics("/class/02-class_files/hod_dow.png", error = FALSE)
```
{{% alert note %}}
...
kableExtra::kbl(join_functions, caption = "A breakdown of `dplyr` join functions.", col.names = NULL) %>% kableExtra::column_spec(1, width = "10em") %>% kableExtra::add_header_above(c("`*_join(x, y) ...`"=1, " "))
```
...
knitr::include_graphics("/class/02-class_files/dist.png", error = FALSE)
```
...
knitr::include_graphics("/class/02-class_files/speeds.png", error = FALSE)
```
...
In the homework you will be tidying some messy derived tables based on the bikeshare data using both of these functions, but we can demonstrate both of these functions in **tidying** the messy `gapminder` data in Table \@ref(tab:gapminder-untidy2). Remember that these data were messy as the observations by gender were spread across the columns:
...
First we need to gather the problematic columns with `pivot_wider`.
