...
Figure \@ref(fig:munzner) is taken from Chapter 5 of @munzner_visualization_2014 and lists the main visual channels with which data might be encoded. The grouping and order of the Figure is meaningful. Channels are grouped according to the tasks to which they are best suited and then ordered according to their effectiveness at supporting those tasks. To the left are **magnitude:order** channels -- those that are best suited to tasks aimed at quantifying data items. Tothe Right are **identify:category** channels -- those that are most suited to supporting tasks that involve isolating, grouping and associating data items.
